...
import pandas as pd
from RGB_YCrCb import RGB2YCrCb, YCrCb2RGB  # 新增导入
...
def train(train_data_ir, train_data_vi):
    ...
    epoch_ir_d_loss_lst = []
    epoch_vi_d_loss_lst = []
    epoch_g_adversarial_loss_lst = []
    epoch_content_loss_lst = []
    epoch_intensity_loss_lst = []
    epoch_texture_loss_lst = []
    epoch_g_loss_lst = []
    ...
    for epoch in tbar:
        ...
        for batch in range(batches):
            ...
            # 新增：YCrCb转换和融合逻辑
            # 1. 将可见光图像转换为YCrCb
            vis_ycrcb = RGB2YCrCb(img_vi)
            vis_y = vis_ycrcb[:, 0:1, :, :]  # Y分量
            vis_cr = vis_ycrcb[:, 1:2, :, :]  # Cr分量
            vis_cb = vis_ycrcb[:, 2:3, :, :]  # Cb分量

            # 2. 将红外图像转换为灰度图（作为Y分量）
            ir_gray = rgb_to_gray(img_ir)

            # 3. 使用生成器融合Y分量
            img_fusion_y = G(ir_gray, vis_y)

            # 4. 重新组合：融合的Y + 原始Cr + 原始Cb
            fused_ycrcb = torch.cat([img_fusion_y, vis_cr, vis_cb], dim=1)

            # 5. 转换回RGB
            img_fusion = YCrCb2RGB(fused_ycrcb)
            ...
            # 转换为灰度图后再进行DWT分解
            img_ir_gray = rgb_to_gray(img_ir)
            img_vi_gray = rgb_to_gray(img_vi)
            img_fusion_gray = rgb_to_gray(img_fusion)

            img_ir_D = DWT_Cat(img_ir_gray)
            img_vi_D = DWT_Cat(img_vi_gray)
            img_fusion_D = DWT_Cat(img_fusion_gray)
            ...
            for _ in range(2):
                ...
                interpolated_ir = Variable(alpha_ir * img_ir_D.data + (1 - alpha_ir) * img_fusion_D.data,
                                           requires_grad=True)
                D_out_interp = D_ir(interpolated_ir)
                grad_ir = torch.autograd.grad(outputs=D_out_interp,
                                              inputs=interpolated_ir,
                                              grad_outputs=torch.ones(D_out_interp.size()).cuda(),
                                              retain_graph=True,
                                              create_graph=True,
                                              only_inputs=True)[0]
            ...
            for _ in range(2):
                ...
                interpolated_vi = Variable(alpha_vi * img_vi_D.data + (1 - alpha_vi) * img_fusion_D.data,
                                           requires_grad=True)
                D_out_interp = D_vi(interpolated_vi)
                grad_vi = torch.autograd.grad(outputs=D_out_interp,
            ...
            # (3) Update G network:
            # ----------------------------------------------------------
            Gen_loss, SSIM_loss, Intensity_loss, Grad_loss = g_content_criterion(img_ir, img_vi, img_fusion)

            lambda_1 = 1

            D_out_ir = D_ir(img_fusion_D)
            D_out_vi = D_vi(img_fusion_D)

            # 生成器的对抗损失
            dir_g_adversarial_loss = -torch.mean(D_out_ir)
            dvi_g_adversarial_loss = -torch.mean(D_out_vi)
            ...
            if (batch + 1) % args.log_interval == 0:
                mesg = "{}\tepoch {}:[{}/{}]\n " \
                    ...
                    time.ctime(), epoch + 1, count, batches,
                                  ...
                                  all_g_adversarial_loss / args.log_interval, Gen_loss.item(), SSIM_loss.item(),
                                  Intensity_loss.item(), Grad_loss.item()
                )
        ...
        # 每个epoch结束后，统计均值并保存
        if len(ir_d_loss_lst) > 0:
            epoch_ir_d_loss_lst.append(np.mean(ir_d_loss_lst))
            epoch_vi_d_loss_lst.append(np.mean(vi_d_loss_lst))
            epoch_g_adversarial_loss_lst.append(np.mean(g_adversarial_loss_lst))
            epoch_content_loss_lst.append(np.mean(content_loss_lst))
            epoch_intensity_loss_lst.append(np.mean(all_intensity_loss_lst))
            epoch_texture_loss_lst.append(np.mean(all_texture_loss_lst))
            epoch_g_loss_lst.append(np.mean(g_loss_lst))
        # 清空log_interval loss list，避免跨epoch累计
        ir_d_loss_lst.clear()
        vi_d_loss_lst.clear()
        g_adversarial_loss_lst.clear()
        content_loss_lst.clear()
        all_intensity_loss_lst.clear()
        all_texture_loss_lst.clear()
        g_loss_lst.clear()
    ...
    # ========== 新增：保存每个epoch的损失为txt并打印 ===========
    loss_dict = {
        'ir_d_loss': epoch_ir_d_loss_lst,
        'vi_d_loss': epoch_vi_d_loss_lst,
        'g_adversarial_loss': epoch_g_adversarial_loss_lst,
        'content_loss': epoch_content_loss_lst,
        'intensity_loss': epoch_intensity_loss_lst,
        'texture_loss': epoch_texture_loss_lst,
        'g_loss': epoch_g_loss_lst
    }
    df = pd.DataFrame(loss_dict)
    loss_txt_path = os.path.join(loss_save_path, 'all_epoch_loss.csv')
    df.to_csv(loss_txt_path, index=False)
